...
class GPT2HeadWithValueModel(GPT2PreTrainedModel):
    ...
    def forward(
        self,
        input_ids=None,
        past_key_values=None,  # Đổi từ `past` thành `past_key_values`
        attention_mask=None,
        token_type_ids=None,
        position_ids=None,
        head_mask=None,
        ...
        transformer_outputs = self.transformer(
            input_ids,
            past_key_values=past_key_values,  # Sửa tham số ở đây
    ...
    with torch.no_grad():  
        for i in range(txt_len):
            ...
            if ref_model is not None:
                ...
                next_token_logits[ref_next_token_probs < 0.01] = -np.inf  # -inf test
