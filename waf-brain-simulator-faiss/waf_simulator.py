...
# --- Build FAISS Index (IVF-HNSW) ---
embedding_dim = embeddings.shape[1]
num_clusters = int(np.sqrt(len(payloads)))
...
ids = np.arange(len(payloads))  # Generate unique IDs
index.add_with_ids(embeddings, ids)
print(f"FAISS index contains {index.ntotal} vectors")

# --- Save FAISS Index and Metadata ---
faiss.write_index(index, "testDB/sqli_faiss.index")

with open("testDB/sqli_metadata.pkl", "wb") as f:
